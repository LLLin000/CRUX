...
final class AnalysisTypesTests: XCTestCase {
    ...
    func testRouteDraftTracksCorrectionsAndManualHold() {
        ...
        let analysis = RouteAnalysis(
            ...
            colorAnalyzerVersion: "lab-mask-v1",
            routeSelectorVersion: SeededRouteSelector.version,
            holds: [AnalyzedHold(detection: detection, referenceLab: LabColor(l: 50, a: 0, b: 0))]
        )
        let selected = RouteDraft(analysis: analysis).selecting(seedIndex: 0)
        XCTAssertEqual(selected.selectedHoldIDs, [2])
        ...
        let manual = ManualHold(id: -1, geometry: geometry, referenceLab: LabColor(l: 50, a: 0, b: 0))
        ...
        XCTAssertEqual(corrected.startHoldID, -1)
